"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TypeCreditService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const type_credit_entity_1 = require("./type-credit.entity");
let TypeCreditService = class TypeCreditService {
    constructor(typeCreditRepository) {
        this.typeCreditRepository = typeCreditRepository;
    }
    async create(createTypeCreditDto) {
        const typeCredit = this.typeCreditRepository.create(createTypeCreditDto);
        return await this.typeCreditRepository.save(typeCredit);
    }
    async findAll() {
        return await this.typeCreditRepository.find();
    }
    async findOne(id) {
        const typeCredit = await this.typeCreditRepository.findOne({
            where: {
                id,
            },
        });
        if (!typeCredit) {
            throw new common_1.NotFoundException(`TypeCredit with ID ${id} not found`);
        }
        return typeCredit;
    }
    async update(id, updateTypeCreditDto) {
        const typeCredit = await this.typeCreditRepository.preload({
            id: id,
            ...updateTypeCreditDto,
        });
        if (!typeCredit) {
            throw new common_1.NotFoundException(`TypeCredit with ID ${id} not found`);
        }
        return await this.typeCreditRepository.save(typeCredit);
    }
    async remove(id) {
        const typeCredit = await this.findOne(id);
        await this.typeCreditRepository.remove(typeCredit);
    }
};
exports.TypeCreditService = TypeCreditService;
exports.TypeCreditService = TypeCreditService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(type_credit_entity_1.TypeCredit)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], TypeCreditService);
//# sourceMappingURL=type-credit.service.js.map