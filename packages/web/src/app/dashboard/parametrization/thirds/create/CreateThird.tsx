...
mutation ( $createAffiiate: InputAffiliateCreate,$createUserInput: UserInput!,$createEmployee: InputEmployeeCreate, $createProvider:Boolean
){
...
function  FormThirdNatural({countries,informationUser,informationAffilate,informationEmployee,isProvider}:{isProvider?:any,countries:any,informationUser?:any,informationAffilate?:any,provider?:any,informationEmployee?:any}){
      
      const {generalInformation,handleGeneralInformation,handleChangeGeneralInformation,setGeneralInformation,handleGeneralNumber}=FormGeneralInformation()
      const {workingInformation,handleChangeWorkingInformation,setWorkingInformation,handleWorkingNumber,handleWorkingInformation}=FormWorkingInformation()
      const [beneficiaryInformation, setBeneficiaryInformation] = useState<Beneficiaries[]>([]);
      const {credential,setCredential,handleChangeCredential}= FormCredential()
 ...
 useEffect(() => {
   ...
   return (
	 ...
	 <div className="  flex flex-col  bg-[#FFFFFF]  rounded-bl-lg rounded-br-lg rounded-tr-lg pt-10 px-3">
		   {indexForm===1 && <GeneralInformation  countries={countries} handleGeneralInformation={handleGeneralInformation} handleNumber={handleGeneralNumber} generalInformation={generalInformation} handleChangeGeneralInformation={handleChangeGeneralInformation} />}
		  {indexForm===2 && 
		  <PersonalInformation countries={countries} handleGeneralInformation={handleGeneralInformation} handleNumber={handleGeneralNumber} 
			generalInformation={generalInformation} handleChangeGeneralInformation={handleChangeGeneralInformation} />
			}

		  {(checkedAffiliate && indexForm===3) &&
		  <>
		     <WorkingInformtaion handleWorkingInformation={handleWorkingInformation} workingInformation={workingInformation} handleNumber={handleWorkingNumber} handleChangeWorkingInformation={handleChangeWorkingInformation} />

		     <BeneficiaryInformation beneficiaryInformation={beneficiaryInformation} setBeneficiaryInformation={setBeneficiaryInformation} />
		  </>
		  }
		  {(checkedEmployee && indexForm===4)&&<CredentialsForm handleChangeCredential={handleChangeCredential} credential={credential} />}
