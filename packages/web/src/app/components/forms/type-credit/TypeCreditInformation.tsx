...
export function TypeCreditForm({setShowModalCreate}:{setShowModalCreate:any}){

    const {data,loading,error}=useQuery(AUXLILIARIES)
    const [accounts,setAccounts]=useState<number[]>([])
    const [createTypeCredit, { data:dataCreate,loading:loadingCreate , error:errorCreate}] = useMutation(CREATE_TYPE_CREDIT);
