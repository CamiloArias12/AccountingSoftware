...
function TableThirds ({affiliates }:{affiliates:Affiliate[]}){
  ...
  const [data,setData] = useState<Affiliate[]>(affiliates)
   ...
   const [deleteUser, {data:deleteData,loading:loadingDelete,error:errorDelete}] = useMutation(DELETE_USER);
	      ...
	      <>
	       <motion.tr  key={row.id} className={`${userSelected===row._valuesCache.identification && " selected "} hover:border-l-4  hover:border-l-[#3C7AC2] `}>
		    ...
		    <>
			   ...
			   setUserSelected(Number(row._valuesCache.identification))
