...
function TableCompany ({companies }:{companies:Company[]}){
   ...
   const [updateStatus, {data:statusData,loading,error}] = useMutation(UPDATE_STATUS);
   const [deleteCompany, {data:deleteData,loading:loadingDelete,error:errorDelete}] = useMutation(DELETE_USER);
        ...
        <table className=" w-full table-fixed  table ">
          ...
          <tbody className=" text-sm">
            {virtualRows.map(virtualRow => {
              ...
              return (
	      <>
	       <motion.tr  key={row.id} className={`${userSelected===row._valuesCache.identification && " selected "} hover:border-l-4  hover:border-l-[#3C7AC2] `}>
                  {row.getVisibleCells().map(cell => {
                    return (
		    <>
                      <td  onClick={() =>{ setShowOptions(true)
			   setCompanySelected(Number(row._valuesCache.identification))
