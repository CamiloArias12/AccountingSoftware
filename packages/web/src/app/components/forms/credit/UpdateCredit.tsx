...
function UpdateCredit({idCredit,setShow}:{idCredit:number, setShow:any}) {

   const {credit,setCredit,handleCreditNumber,handleCreditSelect,handleCredit}= useCredit() 
   const {data:dataUser,loading,error}=useQuery(GET_CREDIT,{
   ...
   useEffect(() =>{
	 ...
	 setCredit({
	    identification:dataUser?.findOneCredit?.affiliate?.user?.identification,
	 ...
	 <div className="flex flex-grow  flex-col   rounded-sm my-10 ">
	    <div className=" flex-grow flex flex-row">
		  ...
		  />
		     ...
		     <UpdateTableAmortization setData={setData} data={data}  setSelected={true}/> 
	        <div className="pt-10 flex justify-end">
		  <div className="pr-4">
		     <Button name="Cancelar" background="border border-[#10417B] text-[#10417B]" />
		  </div>
	        <Button name="Aceptar" background="bg-[#10417B] text-white" onClick={() =>{}}/>
	    </div>
