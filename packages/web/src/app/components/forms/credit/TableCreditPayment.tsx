...
import { Credit, InstallmentPayment } from "@/lib/utils/credit/types";
...
`
function TableCreditsPayment({installmentPayment,dateStart}:{installmentPayment:InstallmentPayment[],dateStart:Date}){
  ...
  const columns = useMemo<ColumnDef<InstallmentPayment>[]>(
   ...
   const [paymentCreditInstallments, { data:dataPayment,loading:loadingPayment, error:errorPayment}] = useMutation(PAYMENT_CREDIT_INSTALLMENT);
  ...
  return (
      ...
      <div className="flex  flex-col bg-white rounded-tr-[20px] rounded-b-[20px] ">	  
       <div className={`flex pr-2 flex-grow flex-col text-input `}>
            ...
            <input type="text" value={concept} className={`bg-white  rounded-sm border h-[30px] `}
