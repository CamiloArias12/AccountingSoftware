...
function TableCredits({credits}:{credits:Credit[]}){
  ...
  const columns = useMemo<ColumnDef<Credit>[]>(
	...
	size:50,
      ...
      },
      {
        ...
        cell : (row:any) => (
	 <div className="py-1">
	    <label className={` py-1 px-4 rounded-[30px] bg-[#FFF1CD] `}> $ {row.getValue()}</label>
	 </div>
	),
	...
	cell : (row:any) => (
	 <div className="py-1">
	    <label className={` py-1 px-4 rounded-[30px] ${row.getValue()==="Aprobado" ? "bg-[#BAF7D0] text-sm  text-[#306E47]":"bg-[#ECFF1C] text-sm" }`}>{row.getValue()}</label>
	 </div>
	),
  ...
  const [data ,setData] = useState<Credit[]>(credits)
   ...
   const [isRefinance, { data:dataRefinance,loading:loadingRefinance , error:errorRefinance}] = useMutation(REFINANCE);
   const [deleteCredit, { data:deleteData,loading:loadingDelete, error:errorDelete}] = useMutation(DELETE_CREDIT);
...
useEffect(() => {
  ...
  return (
      <>
	 {update &&
	    <UpdateCredit idCredit={Number(id)} setShow={setUpdate}  />
        ...
        <table className="h-full w-full table-fixed table ">
          ...
          <tbody className=" ">
            {virtualRows.map(virtualRow => {
              ...
              return (
	      <>
	       <motion.tr  key={row.id} className={ ` ${id===row._valuesCache.id && "selected"} hover:border-l-4  hover:border-l-[#3C7AC2] `}>
