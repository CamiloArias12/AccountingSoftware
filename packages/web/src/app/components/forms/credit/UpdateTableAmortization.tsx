...
function UpdateTableAmortization({setSelected,data,setData}:{data:AmortizationTable[],setData:any,setSelected:any}){
   
   console.log(data)

   const [generateAmortizationChange, ] = useMutation(GENERATE_TABLE_AMORTIZATION_CHANGE);
  ...
  const columns = useMemo<ColumnDef<AmortizationTable>[]>(
	...
	cell : (row:any) => (
	 <div className="py-1">
	    <input className ="bg-transparent text-center" value={row.getValue()} onChange={(e) =>{
	...
	cell : (row:any) => (
	 <div className="py-1">
	    <label className={` py-1 px-4 rounded-[30px] ${row.getValue()==="Pagada" ? "bg-[#BAF7D0] text-sm  text-[#306E47]":"bg-[#FECACA] text-sm" }`}>{row.getValue()==="Pagada" ? "Pagada" :"Pendiente"}</label>
	 </div>
	),
  ...
  return (
      <div>
      <button className="ml-4  text-input flex flex-row rounded-sm bg-[#F2F5FA] p-2 " onClick={handleAmortizationTable}>
	 ...
	 >
	    ...
	    {paddingTop > 0 && (
            ...
            {virtualRows.map(virtualRow => {
              ...
              return (
	      <>
	       <motion.tr  key={row.id} className=" border-b border-b-gray-200 hover:border-l-4 p-2 hover:border-l-[#3C7AC2] ">
