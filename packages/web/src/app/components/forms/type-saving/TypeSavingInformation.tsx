...
export function TypeSavingForm({setShowModalCreate}:{setShowModalCreate:any}){

    const {data,loading,error}=useQuery(AUXLILIARIES)
    const [accounts,setAccounts]=useState<number[]>([])
    const [createTypeSaving, { data:dataCreate,loading:loadingCreate , error:errorCreate}] = useMutation(CREATE_TYPE_SAVING);
   ...
   const handleValueAccounts=(index:number,value:number) => {
    ...
    return (
	 ...
	 >
        ...
        <div className="flex flex-col   w-full h-full">
		  ...
		  {accounts.map((value,index) => (
			...
			<div className="flex-grow mb-2">
			   <Select options={data.getAuxilaryAll} index={index} setValue={handleValueAccounts} />
			</div>
	    ...
	    : (dataCreate?.createTypeSaving===false && showWarning) &&
