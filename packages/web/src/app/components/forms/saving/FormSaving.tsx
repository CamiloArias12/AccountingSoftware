...
function  FormSaving({setShowModalCreate}:{setShowModalCreate:any}){
   ...
   const {saving,handleSaving,handleSavingSelect,handleSavingNumber}= useSaving() 
   const [createSaving, { data:dataCreate, loading: loadingCreate, error: errorCreate}] = useMutation(CREATE_SAVING);
   const [showWarning, setShowWarning] = useState(false);
   const route =useRouter()
   const {data:dataSavings,loading,error}=useQuery(TYPE_SAVINGS_AFFILIATES)
    ...
    <Modal 
	 ...
	 >
      ...
      <div className=" flex-grow flex flex-col bg-white h-full m-2 ">
	    ...
	    <label className="text-center text-white  bg-[#3C7ac2] text-input font-bold mb-2"> Información afiliado</label>
		  <SelectAffiliate 
	    ...
	    <label className="text-center text-white  bg-[#3C7ac2] text-input font-bold my-2"> Información ahorro</label>
	       <SelectAffiliate 
		  ...
		  <Button name="Aceptar" background="bg-[#10417B] text-white" onClick={handleCreateSaving}/>
	       </div>
