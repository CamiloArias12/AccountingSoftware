...
function TableTypeAccount({typeAccounts ,setSelected,setShowModalCreate}:{typeAccounts:GeneralTypeAccount[],setShowModalCreate:any,setSelected:any}){
   

   console.log("Types accounts",typeAccounts)
  const columns = useMemo<ColumnDef<GeneralTypeAccount>[]>(
	...
	cell : (row:any) => (
	 <div className="py-1">
	    <label className={` py-1 px-4 rounded-[30px] ${row.getValue() ? "bg-[#BAF7D0] text-sm  text-[#306E47]":"bg-[#FECACA] text-sm" }`}>{row.getValue() ? "Activa" :"Inactiva"}</label>
	 </div>
	),
  ...
  const [data ] = useState<GeneralTypeAccount[]>(typeAccounts)
